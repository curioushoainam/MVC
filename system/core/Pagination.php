...
class Pagination {
	protected $_config = array(
		'current_page'  => 1, // Trang hiện tại   => get
	    'total_record'  => 1, // Tổng số record    => get
	    'total_page'    => 1, // Tổng số trang             => private
	    'limit'         => 8,// limit             =>get
	    'start'         => 0, // start         
	    'link_full'     => '#',// Link full có dạng như sau: domain/com/page/{page}    =>get 
	    'link_first'    => '#',// Link trang đầu tiên                                  =>get
	    'range'         => 8, // Số button trang tối đa bạn muốn hiển thị 
        'min'           => 0, // Tham số min
    ...
    function html() {
    	...
    	if ($this->_config['total_record'] > $this->_config['limit']){
            ...
            $displayFirst = $this->_config['current_page'] > 1 ? 1 : 1;
            $displayPrev = $this->_config['current_page'] > 1 ? $this->_config['current_page']-1 : 1;

           $p .= '<li><a href="#" class="page" data-page="'.$displayFirst.'" aria-label="Previous"><span aria-hidden="true">&lt;&lt;</span></a></li>';

            $p .= '<li><a href="#" class="page" data-page="'.$displayPrev.'" aria-label="Previous"><span aria-hidden="true">&lt;</span></a></li>';
    		...
    		for ($i=$this->_config['min']; $i<=$this->_config['max']; $i++){
    			...
    			if ($this->_config['current_page'] == $i){
    				...
    				$p .= '<li><a href="#" class="page" data-page="'. $i .'"> '.$i.' </a></li>';
    			}
    		}
    		...
    		if ($this->_config['current_page'] < $this->_config['total_page']){ 
                ...
                $p .= '<li><a href="#" class="page" data-page="'. ($this->_config['current_page'] + 1).'" aria-label="Next"><span aria-hidden="true">&gt;</span></a></li>';
                $p .= '<li><a href="#" class="page" data-page="'. ($this->_config['total_page']).'" aria-label="Next"><span aria-hidden="true">&gt;&gt;</span></a></li>';    			
    		}
    		$p .= '</nav></div>';
    	}
    	return $p;
